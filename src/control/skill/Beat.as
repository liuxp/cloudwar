package control.skill
{
	 ...
	 * 
	 */	
	public class Beat extends SkillBase
	{
		...
		public function Beat(data:Object)
		{
			super(data);
			_atkAI = data.ai_atk;
			_atkVO = _atkAI._vo;
			...
			for each(var i:CharacterBase in dict)
			{
				...
				if(meditor_target && meditor_target.ai)
				{
					...
					if(GameUtil.isArmyAlive(camp, viewId))
					{
						...
						var posX:int = i.x + 180 * -charVO.scaleX;
						if(posX > _ML.camera.x_right) posX = i.x + 180 * charVO.scaleX;
						TweenMax.to(i,0.5,{x:posX, ease:Strong.easeOut});
						
						createEft(_skill.eff_up_enimy, 60, 
							_atkVO.scaleX, targetAI.character.container_temp, _skill.duration);
					}
